...
| 路径 | 方法 | 作用 |
| --- | --- | --- |
| `GET /ping` | 返回 `{ok, blender, queue_size, mcp, snapshots, features}` |
| `POST /exec` | body `{code, scene_name}`，把代码入主线程队列异步执行 |
| `GET /log` | 最近 20 条执行日志 |
| `GET /scene_report` | 返回当前场景 JSON（v1.1.0+，AI 自检用） |
| `GET /viewport_screenshot?max_size=N` | 视口截图 base64（v1.6.6+） |
| `GET /mcp/tools` | OpenAI tools 格式 16 个工具 schema（v2.0.0+） |
| `POST /mcp/call` | body `{tool, args}` 通用工具调度（v2.0.0+） |
| `POST /mcp/<tool_name>` | body `{...args}` 直调对应工具（v2.0.0+） |
| **`GET /blend_summary`** ⭐ | 返回当前 .blend 文件大小 / 物体计数（按 type 分组）/ 集合树 / 渲染设置 overview，用于 Critic 审图前快速摸底（v2.1.0+ Phase F） |
| **`POST /bookmark_state`** ⭐ | body `{name}`，把当前场景以 JSON 形式快照到 Blender 内存（按 name 索引），用于「修复失败回滚」（v2.1.0+ Phase F） |
| **`POST /restore_state`** ⭐ | body `{name}`，从快照恢复——删除自快照以来新增的物体 + 还原已存物体的 transform。已被删的复杂 mesh 几何无法重建会列在 `missing` 字段（v2.1.0+ Phase F） |

**线程安全**：HTTP 在后台线程接收，`bpy.app.timers` 每 0.2s 在主线程 `exec()`，避免 `bpy.data` 跨线程崩溃。

**v2.1.0 软回滚原理（重要）**：`bookmark_state` **不存** mesh 顶点数据（爆内存），只存 `transform / parent / material 名 / vertex_count` 等元数据。所以 `restore_state` 是**软回滚**——能撤销 transform 改动 + 删除自快照以来新增的物体，但已被 AI 误删的复杂 mesh 几何回不来（响应里 missing 字段告知）。建议工作流：① Modeler 改场景前先调 `POST /bookmark_state {name:'before_furniture_fix'}` ② Critic 发现失败时调 `POST /restore_state {name:'before_furniture_fix'}` 软回滚 ③ Modeler 重新尝试。
