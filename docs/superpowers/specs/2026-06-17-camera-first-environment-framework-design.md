...
- 节点式工作流：后续重构 UI 和功能时，整体流程采用节点图，而不是固定表单或单一大按钮。
- 视觉增强可选：部分节点可以选择是否启用视觉模型；启用时先截取 Blender 目标镜头画面再输出，不启用时直接基于文本和结构化 plan 输出。
...
后续重构整个实时渲染 UI 和功能时，主流程应采用节点式工作流。每个节点都要有明确的输入、输出、模型配置、能力要求和日志。

节点类型：

- 必选节点：流程必须经过，例如 Reference Analyzer、Camera Solver、Layer Planner、Shadow Geometry Router、MCP Executor。
- 可选节点：用户可开启或关闭，例如 Blender Camera Vision Check、Atmosphere Refiner、Shadow Softness Refiner、Material Style Critic。
- 视觉增强节点：开启后会先从 Blender 获取目标相机截图或安全余量截图，再调用视觉模型分析。
- 文本/结构化节点：不开启视觉时，直接消费 `reference_analysis.json`、`camera_locked_scene_plan.json` 和上游日志。
- 专用生成节点：不属于 LLM 节点，例如 Hunyuan3D image-to-3D、图片生成模型、素材库检索、PolyHaven 下载。

视觉要求级别：

- `vision_required`：必须使用视觉模型。UI 只提示要求，不主动预检；如果模型/API 在运行时回传不支持图片或格式错误，节点失败并写日志。
- `vision_optional`：视觉增强可开关。开启时截图并走视觉模型；关闭时直接用上游 JSON。
- `vision_not_needed`：不需要视觉，只要求结构化输出、推理、代码或 tool calling。

必须带视觉的节点至少包括：

- Reference Analyzer：没有视觉就无法可靠解析参考图。
- Camera QA Critic：需要看目标相机和安全余量镜头截图。
- 任何读取 Blender 截图的可选视觉节点：只要用户开启，就必须选择视觉模型。

UI 面向有一定基础的用户，不需要过度傻瓜化。每个节点只给简短 tip、能力 badge 和错误日志；用户选错模型时由模型/API 回传错误，系统负责把错误原样结构化进日志。

视觉增强节点的行为规则：

1. 先确认 Blender 已有目标相机和当前场景。
2. 调用 Blender 插件截图目标相机画面，优先使用目标相机，不依赖用户手动视口。
3. 可选抽查安全余量镜头。
4. 把截图和上游 plan 一起交给视觉模型。
5. 输出结构化 JSON patch 或 critique。
6. 所有截图、模型、判断和失败都写入 `decision_log.jsonl`。

不开启视觉增强时：

1. 节点直接读取上游 JSON。
2. 输出下一阶段 JSON 或文本指令。
3. 日志中明确记录 `vision_enabled: false`。

这样用户可以根据成本和模型能力选择：

- 快速模式：只跑核心文本/结构化节点。
- 高质量模式：为关键节点开启视觉增强。
- 省钱模式：只在最终 Camera QA 开启视觉。
- 手动精修模式：用户选择任意节点临时开启视觉复核。

## 模型与生成器路由

系统里有两类“模型”：

- LLM / 视觉 LMM：负责看图、规划、生成 JSON、调用工具、写脚本、做 QA。
- 专用生成模型或服务：负责把某种输入转换成资产，例如 Hunyuan3D 把图片转换成 GLB 白模。

两类模型不能混用：

- Hunyuan3D 不负责规划、审图、写 Blender 脚本或理解整张参考图。
- LLM / 视觉模型不直接承担高成本 mesh 生成，只决定是否需要调用专用生成节点。
- 混元3D 当前作为 image-to-3D 节点使用。若输入只有文字，必须先由图片生成节点或用户参考图提供可用图片，再调用混元3D。

Hunyuan3D 使用规则：

- 只用于镜头中需要近看、参与遮挡、投影、接触或视差的复杂实体。
- 不作为大环境、远山、天空、雾、远景剪影的主路径。
- 输入必须是图片。可以来自用户参考图裁切、Vision Lead 生成的裁切建议、图片生成模型输出，或用户手动上传。
- 输出通常是白模，需要后续由 MCP Executor、素材库、PBR 贴图和灯光进行修饰。
- Hunyuan3D 生成物导入后必须进入 Shadow Geometry Router 复核：是否需要投影、是否需要缩放、是否需要补低模支撑或接触阴影。

节点路由建议：

- `Vision Lead`：LLM/LMM 节点，必须视觉。
- `Scene Planner`：LLM 节点，结构化输出。
- `Image Generator`：可选专用节点，文字转参考图或局部物体图。
- `Hunyuan3D Image-to-3D`：可选专用节点，图片转 GLB。
- `Asset Search / PolyHaven`：可选专用节点，真实素材补强。
- `MCP Executor`：LLM 工具节点，负责把上述产物导入、摆放、修饰。
...
- 能力要求提示。
- 不主动预检模型能力。
- 视觉要求级别提示：必须视觉 / 可选视觉 / 不需要视觉。
- 模型/API 回传错误时写入日志，不静默降级。
